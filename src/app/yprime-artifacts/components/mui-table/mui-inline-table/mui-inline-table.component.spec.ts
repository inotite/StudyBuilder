...
import {MuiInlineTableComponent} from './mui-inline-table.component';
import {CUSTOM_ELEMENTS_SCHEMA, NO_ERRORS_SCHEMA, ChangeDetectorRef, SimpleChange, NgModule} from '@angular/core';
import {FormsModule, ReactiveFormsModule} from '@angular/forms';
import {HttpClientTestingModule} from '@angular/common/http/testing';
import {MaterialUIModule} from '../../../material-ui/material-ui.module';
import {MatTableModule, MatDialogConfig, MatInput, MatSpinner, MatDialogRef, MAT_DIALOG_DATA} from '@angular/material';
...
describe('Inline Table Component', () => {
   let component: MuiInlineTableComponent;
   let fixture: ComponentFixture<MuiInlineTableComponent>;
   ...
   function assignInput(inputElement: HTMLInputElement, text: string) {
      ...
      const de = allInputs.find(item => item.nativeElement.id === inputElement[0].id);
   ...
   function validateTableBlock(pageNumber: number, pageSize: number, totalRecords: number) {
      // validate header
      const headerRow = fixture.nativeElement.querySelectorAll('mat-header-row');
      expect(headerRow.length).toBe(1);

      const headerCols = fixture.nativeElement.querySelectorAll('button.mat-sort-header-button');
      expect(headerCols.length).toBe(2);
      expect(headerCols[0].innerText).toBe('name');
      expect(headerCols[1].innerText).toBe('');

      // validate grid
      const currentSet = (pageNumber - 1) * pageSize;
      const tableRows = fixture.nativeElement.querySelectorAll('mat-row');
      expect(tableRows.length).toBe(totalRecords - currentSet > pageSize ? pageSize : totalRecords - currentSet);
      ...
      for (let index = 0; index < pageSize; index++) {
         ...
         if (counter < totalRecords) {
            ...
            expect(rowCells[0].innerText).toBe('DataItem ' + counter.toString());

            const panelEditable = index % 2 === 0;
            const editActionPanel = rowCells[1].querySelectorAll('.ActionPanelEditButton');
            const editButton = editActionPanel[0].childNodes[0];
            expect(editButton.innerText).toBe('create');
            expect(editButton.title).toBe(panelEditable ? '' : 'Cannot be deleted or modified');
            expect(editButton.disabled).toBe(!panelEditable);

            const deleteActionPanel = rowCells[1].querySelectorAll('.ActionPanelDeleteButton');
            const deleteButton = deleteActionPanel[0].childNodes[0];
            expect(deleteButton.innerText).toBe('delete');
            expect(deleteButton.title).toBe(panelEditable ? '' : 'Cannot be deleted or modified');
   ...
   beforeEach(async(() => {
      TestBed.configureTestingModule({
         declarations: [MuiInlineTableComponent, PipeHelper],
         schemas: [CUSTOM_ELEMENTS_SCHEMA, NO_ERRORS_SCHEMA],
         imports: [
            FormsModule,
            ReactiveFormsModule,
   ...
   beforeEach(async(() => {
      fixture = TestBed.createComponent(MuiInlineTableComponent);
      ...
      it('should display spinner when status is processing', () => {
         ...
         fixture.whenStable().then(() => {
            fixture.whenRenderingDone().then(() => {
               ...
               const spinner = fixture.debugElement.queryAll(By.directive(MatSpinner));
               ...
               // directly call ngOnChanges
               component.ngOnChanges({
                  data: new SimpleChange(null, new Array<InlineTableTestModel>(), false),
                  processingStatus: new SimpleChange(null, processingStatus, false)
               });
               fixture.detectChanges();
               ...
               fixture.whenStable().then(() => {
                  fixture.whenRenderingDone().then(() => {
                     ...
                     const spinnerCtl = fixture.debugElement.queryAll(By.directive(MatSpinner));
      ...
      it('should display no records when no data is provided', () => {
         ...
         fixture.whenStable().then(() => {
            fixture.whenRenderingDone().then(() => {
               ...
               const messageText = fixture.nativeElement.querySelectorAll('.message');
      ...
      it('should display five records on first page with an action panel of edit and delete buttons', () => {
         ...
         fixture.whenStable().then(() => {
            fixture.whenRenderingDone().then(() => {
               ...
               expect(rowCells[0].innerText).toBe('DataItem 0');

               rowCells = tableRows[1].querySelectorAll('mat-cell');
               expect(rowCells[0].innerText).toBe('DataItem 1');

               rowCells = tableRows[2].querySelectorAll('mat-cell');
               expect(rowCells[0].innerText).toBe('DataItem 10');

               rowCells = tableRows[3].querySelectorAll('mat-cell');
               expect(rowCells[0].innerText).toBe('DataItem 11');

               rowCells = tableRows[4].querySelectorAll('mat-cell');
               expect(rowCells[0].innerText).toBe('DataItem 12');
            });
         });
      });
            ...
            fixture.whenRenderingDone().then(() => {
               ...
               fixture.whenStable().then(() => {
                  fixture.whenRenderingDone().then(() => {
                     ...
                     tableRows = fixture.nativeElement.querySelectorAll('mat-row');
                     rowCells = tableRows[0].querySelectorAll('mat-cell');
                     expect(rowCells[0].innerText).toBe('DataItem 9');

                     rowCells = tableRows[1].querySelectorAll('mat-cell');
                     expect(rowCells[0].innerText).toBe('DataItem 8');

                     rowCells = tableRows[2].querySelectorAll('mat-cell');
                     expect(rowCells[0].innerText).toBe('DataItem 7');

                     rowCells = tableRows[3].querySelectorAll('mat-cell');
                     expect(rowCells[0].innerText).toBe('DataItem 6');

                     rowCells = tableRows[4].querySelectorAll('mat-cell');
                     expect(rowCells[0].innerText).toBe('DataItem 5');
                  });
               });
            });
         });
         ...
         for (let index = 0; index < pageSizes.length; index++) {
            ...
            fixture.whenStable().then(() => {
               fixture.whenRenderingDone().then(() => {
                  ...
                  for (let pageNumber = pageCount; pageNumber > 1; pageNumber--) {
                     expect(component.paginator.pageIndex).toBe(pageNumber - 1);
                     validateTableBlock(pageNumber, pageSize, 31);
      ...
      it('should process delete operation on a Yes confirmation', () => {
         ...
         fixture.whenStable().then(() => {
            fixture.whenRenderingDone().then(() => {
               ...
               expect(component.actionClicked.emit).toHaveBeenCalledWith(deleteAction);
            });
         });
      });
      ...
      it('should show edit view when record is edited', () => {
         ...
         fixture.whenStable().then(() => {
            fixture.whenRenderingDone().then(() => {
               ...
               let saveButtonPanel = rowCells[1].querySelectorAll('.ActionPanelSaveButton');
               ...
               fixture.whenStable().then(() => {
                  fixture.whenRenderingDone().then(() => {
                     saveButtonPanel = rowCells[1].querySelectorAll('.ActionPanelSaveButton');
                     ...
                     const cancelButtonPanel = rowCells[1].querySelectorAll('.ActionPanelCancelButton');
            ...
            fixture.whenRenderingDone().then(() => {
               ...
               fixture.whenStable().then(() => {
                  fixture.whenRenderingDone().then(() => {
                     ...
                     const saveButtonPanel = rowCells[1].querySelectorAll('.ActionPanelSaveButton');
                     const saveButton = saveButtonPanel[0].childNodes[0];
                     ...
                     fixture.whenStable().then(() => {
                        fixture.whenRenderingDone().then(() => {
                           ...
                           const validationMessage = rowCells[1].querySelectorAll('.showErrorMessage');
